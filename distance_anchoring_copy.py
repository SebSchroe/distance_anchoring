...
# main code to execute block by block
...
def training(sub_id, cond_id, block_id, task_id, n_reps, isi):
    ...
    if task_id == 1: # use all speakers
        speaker_dic = speaker_dict
    elif task_id == 2: # only use speakers with index 1 to 6
        speaker_dic = {k: v for k, v in speaker_dict.items() if 1 <= k <= 6}
    else:
    ...
    for trial in seq:
        ...
        # equalize USO and play it from speaker corresponding to to slider value
