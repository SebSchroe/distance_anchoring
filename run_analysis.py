...
#TODO: next steps: work with signed_error and do simple t-test statistic at different speaker_distances (check for normality -> if yes do t-test)

# %% plot individual raw data per participant and save the plot
for sub_id in sub_ids:
    analysis.plot_data_per_sub(df=cleaned_df, sub_id=sub_id, x="speaker_distance", y="signed_error", baseline="zero", save=True)

# %% observe questionnaire at its own
analysis.observe_questionnaire(df=questionnaire_df, x="cond_id", y="age", hue="gender")

# %% plot averaged data
analysis.plot_averaged_data(df=cleaned_df, x="speaker_distance", y="absolute_error")
...
distribution_df = means_df[(means_df["cond_id"] == 2) & (means_df["block_id"] == 6) & (means_df["speaker_distance"] == 12)]
